    ...
    def process_command(self, cmd):
        ...
        # --- DIAGNOSTICS ---
            ...
            if args and args[0] == "MIC":
                r = self.bot.mic.test()
                return f"OK: MIC init={r['init']} read={r['read_ok']} signal={r['has_signal']} avg={r['avg_level']} max={r['max_level']} pass={r['pass']} err={r['error']}"
            elif args and args[0] == "SPK":
                r = self.bot.speaker.test()
                return f"OK: SPK init={r['init']} play={r['play_ok']} pass={r['pass']} err={r['error']}"
            elif args and args[0] == "AUDIO":
                ...
                return "ERR: TEST needs AUDIO, MIC, or SPK"
